...
export const getCollectionAndDownloadStatus = async () => {
  ...
  const collectedMovieCode = new Set(collectedMovies.map(item => item.movieCode));
  const downloadMovieCode = new Set(downloadMovies.map(item => item.movieCode));

  return { collectedMovieCode, downloadMovieCode };
};

export const formatMovie = (movie, { collectedMovieCode, downloadMovieCode }) => {
